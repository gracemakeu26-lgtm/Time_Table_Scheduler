...
const CourseDetailModal = ({ course, isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <div
        className='fixed inset-0 bg-white bg-opacity-80 backdrop-blur-sm z-40 transition-opacity'
        onClick={onClose}
      ></div>
      ...
      <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
        <div className='bg-white rounded-lg shadow-2xl max-w-md w-full border border-gray-200 transform transition-all'>
          ...
          <div className='bg-gray-900 text-white px-6 py-4 rounded-t-lg'>
            <h2 className='text-xl font-bold text-white mb-1'>{course.course}</h2>
            {course.courseCode && (
              <p className='text-sm text-gray-300 font-medium'>{course.courseCode}</p>
            )}
            <p className='text-sm text-gray-300 font-medium mt-1'>{course.type}</p>
          </div>
          ...
          <div className='px-6 py-4 space-y-4'>
            ...
            <div className='flex items-center gap-3 pb-3 border-b border-gray-200'>
              <div className={`w-3 h-3 rounded-full ${
                course.type === 'Lecture' ? 'bg-blue-600' :
                course.type === 'Lab Work' ? 'bg-emerald-600' :
                course.type === 'Tutorial' ? 'bg-indigo-600' : 'bg-gray-400'
              }`}></div>
            ...
            <div className='space-y-1 pb-3 border-b border-gray-200'>
              ...
              <p className='text-base font-semibold text-gray-900'>{course.room}</p>
            </div>
            ...
            <div className='space-y-1 pb-3 border-b border-gray-200'>
              ...
              <p className='text-base font-semibold text-gray-900'>{course.time}</p>
            </div>
            ...
            <div className='space-y-1'>
              ...
              <p className='text-base font-semibold text-gray-900'>{course.day}</p>
            </div>
          </div>
